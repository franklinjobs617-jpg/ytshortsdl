...
const Header = () => {
    ...
    const creditsInfo = useMemo(() => {
        ...
        const remDownload = getRemaining(limits.download, usage?.downloadCount || 0);
        const remExtract = getRemaining(limits.extract, usage?.extractionCount || 0);
        const remSummary = getRemaining(limits.summary, usage?.summaryCount || 0);

        const details = [
            { key: 'download', label: 'Shorts Downloads', icon: <Download size={14} className="text-blue-500" />, value: remDownload, isUnlimited: limits.download > 9000 },
            { key: 'extract', label: 'Audio & Script', icon: <FileText size={14} className="text-purple-500" />, value: remExtract, isUnlimited: limits.extract > 9000 },
            { key: 'summary', label: 'AI Summaries', icon: <BrainCircuit size={14} className="text-pink-500" />, value: remSummary, isUnlimited: limits.summary > 9000 },
        ];
        ...
        if (plan === 'ELITE') totalDisplay = 'Elite';
            ...
            const n1 = typeof remDownload === 'number' ? remDownload : 0;
            const n2 = typeof remExtract === 'number' ? remExtract : 0;
            const n3 = typeof remSummary === 'number' ? remSummary : 0;
            totalDisplay = n1 + n2 + n3;
        }
                    ...
                    <div className="p-3 bg-slate-50 border-t border-slate-100 space-y-2">
                        ...
                        {/* {!isLoggedIn && (
                            ...
                            >
                                ...
                                <div className="bg-white/20 px-2 py-1 rounded text-[10px] font-black group-hover/login:bg-white/30 transition-colors">
                                    FREE
                                </div>
                            </button>
                        )} */}
                        ...
                        {creditsInfo.plan === 'FREE' && (
                            <Link
                                href="/pricing"
                                onClick={() => setShowCreditsBubble(false)}
                                className="group/btn flex items-center justify-between w-full p-3 bg-slate-900 hover:bg-red-600 text-white rounded-xl transition-all shadow-md active:scale-95"
                            >
                                <div className="flex items-center gap-3 text-left">
    ...
    return (
        <>
            <header className="py-4 sticky top-0 bg-white/90 backdrop-blur-md z-50 border-b border-slate-200/50 transition-all duration-300 supports-[backdrop-filter]:bg-white/60">
                <nav className="flex justify-between items-center px-4 md:px-8 mx-auto">
                    ...
                    <div className="hidden lg:flex items-center space-x-7">
                            ...
                            item.type === 'dropdown' ? (
                                <div key={idx} className="relative group py-2">
                                    <button className={`font-medium transition-colors flex items-center gap-1.5 ${isChildActive(item.links!) ? 'text-red-600' : 'text-slate-600 hover:text-red-600'}`}>
                                        {item.label}
                                        <ChevronDown className="w-3.5 h-3.5 opacity-50 group-hover:rotate-180 transition-transform duration-300" strokeWidth={2.5} />
                                    </button>
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            {isLoggedIn ? (
                                ...
                                >
                                    {isLoggingIn ? <><Loader2 className="w-4 h-4 animate-spin" /><span className="text-xs">Wait...</span></> : "Sign In"}
                                </button>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="flex lg:hidden items-center gap-3">
                        <CreditsBadge />
                        <button onClick={() => setIsDrawerOpen(true)} className="p-2 text-slate-600 hover:bg-slate-100 rounded-lg">
                ...
                <nav className="p-6 space-y-2">
                    <div className="mb-8 pb-6 border-b border-slate-100 space-y-4">
                        {isLoggedIn ? <UserProfileUI /> : (
                            <button onClick={login} disabled={isLoggingIn} className="w-full flex items-center justify-center gap-2 py-4 rounded-2xl font-bold bg-slate-900 text-white">
